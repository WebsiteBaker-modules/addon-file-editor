...
function readFtpSettings()
{
        ...
        return ($results && $row = $results->fetchRow(MYSQLI_ASSOC)) ? $row : false;
}
...
function getAddons($force_reload = false)
{
    ...
    while ($results && $row = $results->fetchRow(MYSQLI_ASSOC)) {
...
function getAddonInfos($addon_id_dir)
{
        ...
        $results = $database->query($sql);
        if ($results && $row = $results->fetchRow(MYSQLI_ASSOC)) return $row;
        return false;
}
...
function getReadmeUrl($afe_version) {
    ...
    $loader = new Twig_Loader_Filesystem(WB_PATH.'/'.$sAddonRel.'/templates');
    ...
    ob_start();
    include WB_PATH.$sAddonRel.'/templates/Readme.md.html';
    $sMessage = ob_get_clean();
    $aTwigData = array(
          'lang'          => $aLang,
          'Message'       => $sMessage,
          'HelpFile'      => 'Readme.md.html',
          'AddonThemeUrl' => WB_URL.$sAddonRel.'/templates',
          );

//    $tpl = $twig->loadTemplate('ModalDiakogBox.twig');
//    $tpl->display($data);
//    return WB_URL.$sAddonRel.'/templates/Readme.md.html';
    echo $twig->render('ModalDiakogBox.twig', $aTwigData);
}
