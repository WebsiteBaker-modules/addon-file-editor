...
class HTTP_Download
{
    ...
    public function setGzip($gzip = false)
    {
        ...
        if ($gzip && !$this->PEAR->loadExtension('zlib')){
            return $this->PEAR->raiseError(
    ...
    public function guessContentType()
    {
        ...
        if (class_exists('MIME_Type') || @include_once 'MIME/Type.php') {
            if ($this->PEAR->isError($mime_type = MIME_Type::autoDetect($this->file))) {
                return $this->PEAR->raiseError($mime_type->getMessage(),
                    HTTP_DOWNLOAD_E_INVALID_CONTENT_TYPE);
            }
    ...
    public function send($autoSetContentDisposition = true)
    {
        ...
        if ($end != '*' && $this->isRangeRequest()) {
            ...
            } elseif ($this->PEAR->isError($chunks)) {
                ob_end_clean();
    ...
    public function getChunks()
    {
        ...
        if (!$satisfiable) {
            $error = $this->PEAR->raiseError('Error processing range request',
    ...
    {
        $error = null;
        if ($this->PEAR->isError($this->_error)) {
            $error = $this->_error;
            $this->_error = null;
        }
        return $error;
